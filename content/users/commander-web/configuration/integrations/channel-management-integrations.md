...
5. Create `Channel Manager Rate`s (Make sure that none of the rates in Channel Manager are derived from another rate).
	...
	- For more detail see [Map Channel Manager Rates](#how-to-map-channel-manager-rates).
...
2. The rate exist only in Channel Manager
	- This helps when there is a special Channel Manager Rate set can't be achieved in Mews - e.g. some package rate.
	- Such Channel Manager Rate must be connected to rate in Mews to achieve proper reservation delivery.
	- There can be mmore such Channel Manager Rate connected to 1 rate in Mews.
...
- **Download reservations** - Mews Commander requests the Channel Manager for all pending reservations. After processing the reservations, Mews Commander confirms the reservations.
...
- The integration is disabled or the `Download Reservations` or `Ping Notificiation` or `Create Reservation` method is disabled (depends on channel manager which operation is used to obtain reservations).
   - In this case enable the integraton/operation and wait for the job to get it (if operation is `Download Reservations`). Otherwise ask channel manager to resend the reservation.
