...
Provided css styles are optional and can be completely replaced with custom css. The styles also come in two versions (current or edge) and if you decide to use them, their version should match version of the script. Moreover there are two variants of the styles: **basic** and for opening in **overlay**.
